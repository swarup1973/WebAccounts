...
$(document).ready(function () {
    ...
    BudgetGroupObject.do_loaddata();

    
    //BudgetGroupObject.do_loaddata();
    BudgetGroupObject.do_getUserPagepermission();
...
var BudgetGroupObject = {
    ...
    do_loaddata: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            ...
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", RowId: -1, BudGrpCd: BudGrpCd, BudGrpDesc: BudGrpDesc, LedTypeCd: LedTypeCd, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                  
                    BudgetGroupObject.do_populateData(obj);
                },
                error: function (err) {
    ...
    do_populateData: (obj) => {
        ...
        var editor = new $.fn.dataTable.Editor({
            table: "#addressbook",
            fields: [
                { label: "BudGrpCd", name: "BudGrpCd" },
                { label: "BudGrpDesc", name: "BudGrpDesc" },
                { label: "LedTypeCd", name: "LedTypeCd" }
        ...
        roletable.dataTable({
            ...
            buttons: [
                ...
                {
                    add: "Ledger Account Mapping", text: 'Ledger Account Mapping', editor: editor, action: function () { otherWindow($('.selected').attr('rowid'), 'map'); },
                    attr: {
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                ...
                if (!BudgetGroupObject._deleteperm[0]) {
        ...
        if (!BudgetGroupObject._createperm[0]) {
        ...
        if (!BudgetGroupObject._editperm[0]) {
        ...
        if (!BudgetGroupObject._vieweperm[0]) {
        ...
        if (!BudgetGroupObject._accountmapvieweperm[0]) {
    ...
    do_getUserPagepermission: () => {
        MainObject.do_getuserpageaccess(BudgetGroupObject);
        
        BudgetGroupObject._vieweperm = MainObject.do_IsActionMenuPermission(BudgetGroupObject.access, 'LEDGER AC GROUP FOR BUDGET', 'view');
        BudgetGroupObject._createperm = MainObject.do_IsActionMenuPermission(BudgetGroupObject.access, 'LEDGER AC GROUP FOR BUDGET', 'create');
        BudgetGroupObject._editperm = MainObject.do_IsActionMenuPermission(BudgetGroupObject.access, 'LEDGER AC GROUP FOR BUDGET', 'edit');
        BudgetGroupObject._deleteperm = MainObject.do_IsActionMenuPermission(BudgetGroupObject.access, 'LEDGER AC GROUP FOR BUDGET', 'delete');
        BudgetGroupObject._accountmapvieweperm = MainObject.do_IsActionMenuPermission(BudgetGroupObject.access, 'LEDGER ACCOUNT MAPPING', 'view');
        BudgetGroupObject._accountmapmenuid = MainObject.do_IsActionMenuPermission(BudgetGroupObject.access, 'LEDGER ACCOUNT MAPPING', 'menuid');
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            ...
            url: apiurl + 'api/GeneralLedgerBudgetGroupOperation',
            type: 'POST',
            data: { p_mode: "edit", RowId: id, BudGrpCd: BudGrpCd, BudGrpDesc: BudGrpDesc, LedTypeCd: LedTypeCd, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            ...
            success: function (response) {
                ...
                BudgetGroupObject.rowid = response[0].RowId;
                 $("#txtCode").val(response[0].BudGrpCd);
                 $("#txtName").val(response[0].BudGrpDesc);
                 $("#ddType").val(response[0].LedTypeCd);
    ...
    do_loadwarehouselocation: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", LocationCd: '', CoCd: $("#ddlCompany").val(), LocationDesc: '', WhId: BudgetGroupObject.rowid, AisleNo: -1, RackNo: -1, SelfNo: -1, BinNo: "-1", created_by: $("#txt").val(), RowId: -1 },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    BudgetGroupObject.do_loadwarehouselocationData(obj);
                },
                error: function () {
...
function saveFinal(imgId) {
    ...
    created_by = $("#txt").val();
    CoCd = $("#ddlCompany").val();
    BudGrpCd = $("#txtCode").val();
    BudGrpDesc = $("#txtName").val();
    LedTypeCd = $("#ddType").val();
   
    
    if (parseInt(BudgetGroupObject.rowid) > 0) {
        $.ajax({

            url: apiurl + 'api/GeneralLedgerBudgetGroupOperation',
            type: 'POST',
            data: { p_mode: "update", RowId: parseInt(BudgetGroupObject.rowid), BudGrpCd: BudGrpCd, BudGrpDesc: BudGrpDesc, LedTypeCd: LedTypeCd, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
                ...
                if (response[0].msg == "true") {
                    ...
                    $.alertable.alert(`Data updated successfully.`, ``, `Ok`, ``).then(function () {
                        window.location = "ledger-acccount-group-for-budget.aspx";
                    });
                }
                else {
        ...
        $.ajax({
            url: apiurl + 'api/GeneralLedgerBudgetGroupOperation',
            type: 'POST',
            data: { p_mode: "create", RowId: parseInt(BudgetGroupObject.rowid), BudGrpCd: BudGrpCd, BudGrpDesc: BudGrpDesc, LedTypeCd: LedTypeCd, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
...
var otherWindow = function (countryCd, mode) {
    ...
    if (localStorage.budgetgrouprowid) {
        if (mode == "map") {
            ...
            localStorage.menu_id_premission = BudgetGroupObject._accountmapmenuid[1];
            localStorage.clickedmenu_id = BudgetGroupObject._accountmapmenuid[1];
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == "viewlocation") {
        BudgetGroupObject.rowid = rowId;
        showmodalview();

    }
    if (mode == 'add') {
        companylogo = "";
        BudgetGroupObject.rowid = '-1';
        showmodal();
        $('.modal-title').html('Ledger AC Group for Budget - New');
    ...
    if (mode == 'edit') {
        ...
        $('.modal-title').html('Ledger AC Group for Budget - Edit');
        $('#cbBlock').show();
        datablank();
        if (!BudgetGroupObject._deleteperm[0]) {
        ...
        $('#txtCode').prop("disabled", true);
        BudgetGroupObject.rowid = rowId;
        BudgetGroupObject.do_loaddataedit(rowId);
        
    }
    else if(mode == 'view') {
        showmodal();
        datablank();
        $('.modal-title').html('Ledger AC Group for Budget - View');
        $('#cbBlock').show();
       // $('#btnDelete').show();
        // $('#btnEdit').show();
        if (!BudgetGroupObject._deleteperm[0]) {
...
function PopulateState() {
    BudgetGroupObject.do_populateLocationDropdown('STATE', $('#ddPincodeCountry').val(), $('#ddPincodeCounty'));
}
function PopulateCity() {
    BudgetGroupObject.do_populateLocationDropdown('CITY', $('#ddPincodeCounty').val(), $('#ddPincodeCity'));
}
