...
var savedata = function () {
    ...
    else if ($('#ddAccGrp').val() < 1) {
        validate = false;
        gotfocus($("#ddAccGrp"));
        $.alertable.alert(`AC Group for Budget required.`);
        //$("#txtName").focus();
        return false;
    }
    else if ($("#txtThreshold").val() >100) {
        validate = false;
        gotfocus($("#txtThreshold"));
        $.alertable.alert(`Threshold Limit (%) is Max <=100%. not more than that.`);
        //$("#txtName").focus();
        return false;
    }
