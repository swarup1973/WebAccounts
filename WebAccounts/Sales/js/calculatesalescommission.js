...
$(document).ready(function () {
    ...
    SalescommissionCalculateoverviewObject.do_populateMasterDropdown('JOURNAL');

    
    //SalescommissionCalculateoverviewObject.do_loaddata();
    //SalescommissionCalculateoverviewObject.do_getUserPagepermission();
...
var SalescommissionCalculateoverviewObject = {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var obj = response;
                    SalescommissionCalculateoverviewObject.do_populateData(obj);
                },
                error: function (err) {
    ...
    do_populateData: (obj) => {
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                ...
                if (!SalescommissionCalculateoverviewObject._deleteperm[0]) {
        ...
        if (!SalescommissionCalculateoverviewObject._createperm[0]) {
        ...
        if (!SalescommissionCalculateoverviewObject._editperm[0]) {
        ...
        if (!SalescommissionCalculateoverviewObject._vieweperm[0]) {
    ...
    do_getUserPagepermission: () => {
        MainObject.do_getuserpageaccess(SalescommissionCalculateoverviewObject);
        
        SalescommissionCalculateoverviewObject._vieweperm = MainObject.do_IsActionMenuPermission(SalescommissionCalculateoverviewObject.access, 'SALES COMMISSION SET-UP', 'view');
        SalescommissionCalculateoverviewObject._createperm = MainObject.do_IsActionMenuPermission(SalescommissionCalculateoverviewObject.access, 'SALES COMMISSION SET-UP', 'create');
        SalescommissionCalculateoverviewObject._editperm = MainObject.do_IsActionMenuPermission(SalescommissionCalculateoverviewObject.access, 'SALES COMMISSION SET-UP', 'edit');
        SalescommissionCalculateoverviewObject._deleteperm = MainObject.do_IsActionMenuPermission(SalescommissionCalculateoverviewObject.access, 'SALES COMMISSION SET-UP', 'delete');
    ...
    do_populateMasterDropdown: (ctype) => {
        ...
        $.ajax({
            url: apiurl + 'api/GetMasterDropdownForCalculateSalesCommision',
            type: 'POST',
            ...
            success: function (response) {
                ...
                if (ctype == 'JOURNAL') {
                    $("#ddJournal").html(_html.join(""));      
                    SalescommissionCalculateoverviewObject.do_populateMasterDropdown('COMMISION');

                }
                if (ctype == 'COMMISION') {
                    $("#ddCommission").html(_html.join(""));
                }
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                SalescommissionCalculateoverviewObject.rowid = response[0].RowId;
                ...
                if (svalp == 1 || svalp == 2) {
                    ...
                    //SalescommissionCalculateoverviewObject.do_populateSalesDropdown(svalp, response[0].SalesPersonRelation);  
    ...
    do_loadwarehouselocation: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", LocationCd: '', CoCd: $("#ddlCompany").val(), LocationDesc: '', WhId: SalescommissionCalculateoverviewObject.rowid, AisleNo: -1, RackNo: -1, SelfNo: -1, BinNo: "-1", created_by: $("#txt").val(), RowId: -1 },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    SalescommissionCalculateoverviewObject.do_loadwarehouselocationData(obj);
                },
                error: function () {
...
function saveFinal(imgId) {
    ...
    if (parseInt(SalescommissionCalculateoverviewObject.rowid) > 0) {
        $.ajax({

            url: apiurl + 'api/SalesCommissionSetupoperation',
            type: 'POST',
            data: { pmode: "update", RowId: parseInt(SalescommissionCalculateoverviewObject.rowid), CoCd: CoCd, IsBlock: IsBlock, IsStop: isStop, created_by: created_by, creator_MAC_add: creator_MAC_add, CommCd: CommCd, CommDesc: CommDesc, CalcBasis: CalcBasis, CalcRelation: CalcRelation, SalesPerson: SalesPerson, SalesPersonRelation: SalesPersonRelation, CommPercent: CommPercent, BaseValueForCalc: BaseValueForCalc, ValidFrom: ValidFrom, ValidTill: ValidTill, AcCd_CommAcExpns: AcCd_CommAcExpns, AcCd_CommAcLiab: AcCd_CommAcLiab },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
...
var otherWindow = function (countryCd, mode) {
    ...
    if (localStorage.salesgrouprowid) {
        if (mode == "salesperson") {
            ...
            localStorage.clickedmenu_id = SalescommissionCalculateoverviewObject._salesmenuid[1];
            localStorage.menu_id_premission = SalescommissionCalculateoverviewObject._salesmenuid[1];
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == 'add') {
        companylogo = "";
        SalescommissionCalculateoverviewObject.rowid = '-1';
        showmodal();
    ...
    if (mode == 'edit') {
        ...
        SalescommissionCalculateoverviewObject.rowid = rowId;
        
        SalescommissionCalculateoverviewObject.do_loaddataedit(rowId);
...
function PopulateState() {
    SalescommissionCalculateoverviewObject.do_populateLocationDropdown('STATE', $('#ddPincodeCountry').val(), $('#ddPincodeCounty'));
}
function PopulateCity() {
    SalescommissionCalculateoverviewObject.do_populateLocationDropdown('CITY', $('#ddPincodeCounty').val(), $('#ddPincodeCity'));
}
...
function getSalesCommissionOverViewdetails() {
    var created_by, creator_MAC_add, CoCd, IsBlock, CommCd, CommDesc, CalcBasis, CalcRelation, SalesPerson, SalesPersonRelation, CommPercent, BaseValueForCalc, ValidFrom, ValidTill, AcCd_CommAcExpns, AcCd_CommAcLiab, isStop;
    IsBlock = 0
    isStop = 0
    creator_MAC_add = ipaddress;
    ...
    AcCd_CommAcExpns = 0;
    AcCd_CommAcLiab = 0;
    var rowid = $("#ddCommission").val();
    $.ajax({

        url: apiurl + 'api/FetchSalesCommissionSetupByRowId',
        type: 'POST',
        data: { RowId: rowid},
        ...
        success: function (response) {
            ...
            $("#txtDesc").val(response[0].CommDesc);
            $("#txtPercent").val(response[0].CommPercent);
            $("#txtCalculationBasis").val(response[0].CalcBasis);
            $("#txtBase").val(response[0].BaseValueForCalc);

            $("#txtCalculationRelation").val(response[0].CalcRelation);
            $("#txtPerson").val(response[0].SalesPerson);
            $("#txtPersonRelation").val(response[0].SalesPersonRelation);
            $("#txtFrom").val(response[0].ValidFrom);
            $("#txtTill").val(response[0].ValidTill);
            $("#txtExpenseAC").val(response[0].AcCd_CommAcExpns);
            $("#txtLiabilityAC").val(response[0].AcCd_CommAcLiab);
