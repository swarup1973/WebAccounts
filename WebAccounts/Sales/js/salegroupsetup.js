...
$(document).ready(function () {
    ...
    SaleGroupSetupObject.do_loaddata();

    
    //SaleGroupSetupObject.do_loaddata();
    SaleGroupSetupObject.do_getUserPagepermission();
...
var SaleGroupSetupObject = {
    ...
    do_loaddata: () => {
        var issBlock = 0
        var created_by, creator_MAC_add, SaleGrpCd, CoCd, GroupDesc, IsBlock;
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            ...
            $.ajax({
                url: apiurl + 'api/SalesSalesGroupOperation',
                type: 'POST',
                data: { p_mode: "getlist", RowId: -1, SaleGrpCd: SaleGrpCd, GroupDesc: GroupDesc, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    SaleGroupSetupObject.do_populateData(obj);
                },
                error: function (err) {
    ...
    do_populateData: (obj) => {
        ...
        var editor = new $.fn.dataTable.Editor({
            table: "#addressbook",
            fields: [
                { label: "SaleGrpCd", name: "SaleGrpCd" },
        ...
        roletable.dataTable({
            ...
            buttons: [
                ...
                {
                    add: "setup", text: 'Add Sales Person/Representative', editor: editor, action: function () { otherWindow($('.selected').attr('rowid'), 'salesperson'); },
                    attr: {
                        title: 'Add Sales Person/Representative',
                        id: 'country_overview_View_setup'
                    }
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                ...
                if (!SaleGroupSetupObject._deleteperm[0]) {
        ...
        if (!SaleGroupSetupObject._createperm[0]) {
        ...
        if (!SaleGroupSetupObject._editperm[0]) {
        ...
        if (!SaleGroupSetupObject._vieweperm[0]) {
        ...
        if (!SaleGroupSetupObject._salesperm[0]) {
            $('#country_overview_View_setup').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#country_overview_View_setup').prop("disabled", true);
            $('#country_overview_View_setup').attr('title', 'do not have permission to view !!!');
    ...
    do_getUserPagepermission: () => {
        MainObject.do_getuserpageaccess(SaleGroupSetupObject);
        
        SaleGroupSetupObject._vieweperm = MainObject.do_IsActionMenuPermission(SaleGroupSetupObject.access, 'SALES GROUP', 'view');
        SaleGroupSetupObject._createperm = MainObject.do_IsActionMenuPermission(SaleGroupSetupObject.access, 'SALES GROUP', 'create');
        SaleGroupSetupObject._editperm = MainObject.do_IsActionMenuPermission(SaleGroupSetupObject.access, 'SALES GROUP', 'edit');
        SaleGroupSetupObject._deleteperm = MainObject.do_IsActionMenuPermission(SaleGroupSetupObject.access, 'SALES GROUP', 'delete');
        
        SaleGroupSetupObject._salesperm = MainObject.do_IsActionMenuPermission(SaleGroupSetupObject.access, 'Add Sales Person/Representative', 'view');
        SaleGroupSetupObject._salesmenuid = MainObject.do_IsActionMenuPermission(SaleGroupSetupObject.access, 'Add Sales Person/Representative', 'menuid');
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            url: apiurl + 'api/SalesSalesGroupOperation',
            type: 'POST',
            data: { p_mode: "edit", RowId: id, SaleGrpCd: SaleGrpCd, GroupDesc: GroupDesc, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            ...
            success: function (response) {
                ...
                SaleGroupSetupObject.rowid = response[0].RowId;
                $("#txtCode").val(response[0].SaleGrpCd);
    ...
    do_loadwarehouselocation: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", LocationCd: '', CoCd: $("#ddlCompany").val(), LocationDesc: '', WhId: SaleGroupSetupObject.rowid, AisleNo: -1, RackNo: -1, SelfNo: -1, BinNo: "-1", created_by: $("#txt").val(), RowId: -1 },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    SaleGroupSetupObject.do_loadwarehouselocationData(obj);
                },
                error: function () {
...
function saveFinal(imgId) {
    ...
    created_by = $("#txt").val();
    CoCd = $("#ddlCompany").val();
    SaleGrpCd = $("#txtCode").val();
    GroupDesc = $("#txtDesc").val();
    
   
    
    if (parseInt(SaleGroupSetupObject.rowid) > 0) {
        $.ajax({

            url: apiurl + 'api/SalesSalesGroupOperation',
            type: 'POST',
            data: { p_mode: "update", RowId: parseInt(SaleGroupSetupObject.rowid), SaleGrpCd: SaleGrpCd, GroupDesc: GroupDesc, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
                ...
                if (response[0].msg == "true") {
                    ...
                    $.alertable.alert(`Data updated successfully.`, ``, `Ok`, ``).then(function () {
                        window.location = "sales-group-setup.aspx";
                    });
                }
                else {
        ...
        $.ajax({
            url: apiurl + 'api/SalesSalesGroupOperation',
            type: 'POST',
            data: { p_mode: "create", RowId: -1, SaleGrpCd: SaleGrpCd, GroupDesc: GroupDesc, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
...
var otherWindow = function (countryCd, mode) {
    ...
    if (localStorage.salesgrouprowid) {
        if (mode == "salesperson") {
            ...
            localStorage.clickedmenu_id = SaleGroupSetupObject._salesmenuid[1];
            localStorage.menu_id_premission = SaleGroupSetupObject._salesmenuid[1];
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == 'add') {
        companylogo = "";
        SaleGroupSetupObject.rowid = '-1';
        showmodal();
        $('.modal-title').html('Sales Group - New');
    ...
    if (mode == 'edit') {
        ...
        $('.modal-title').html('Sales Group - Edit');
        $('#cbBlock').show();
        datablank();
        if (!SaleGroupSetupObject._deleteperm[0]) {
        ...
        $('#txtCode').prop("disabled", true);
        SaleGroupSetupObject.rowid = rowId;
        SaleGroupSetupObject.do_loaddataedit(rowId);
        
    }
    else if(mode == 'view') {
        showmodal();
        datablank();
        $('.modal-title').html('Sales Group - View');
        $('#cbBlock').show();
       // $('#btnDelete').show();
        // $('#btnEdit').show();
        if (!SaleGroupSetupObject._deleteperm[0]) {
            ...
            .then(
                function () {
                    ...
                    $.ajax({
                        ...
                        url: apiurl + 'api/SalesSalesGroupOperation',
                        type: 'POST',
                        data: { p_mode: "delete", RowId: rowId, SaleGrpCd: SaleGrpCd, GroupDesc: GroupDesc, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
                        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                        success: function (response) {
...
function PopulateState() {
    SaleGroupSetupObject.do_populateLocationDropdown('STATE', $('#ddPincodeCountry').val(), $('#ddPincodeCounty'));
}
function PopulateCity() {
    SaleGroupSetupObject.do_populateLocationDropdown('CITY', $('#ddPincodeCounty').val(), $('#ddPincodeCity'));
}
