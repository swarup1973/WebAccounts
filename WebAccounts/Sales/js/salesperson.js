...
$(document).ready(function () {
    ...
    SalesPersonObject.do_populateUserDropdown();
    //SalesPersonObject.do_loaddata();
    //409
   
    //SalesPersonObject.do_loaddata();
    SalesPersonObject.do_getUserPagepermission();
...
var SalesPersonObject = {
    ...
    do_loaddata: () => {
        var issBlock = 0
        var created_by, creator_MAC_add, SaleGrpId, CoCd, EmpId, IsBlock;
        ...
        created_by = $("#txt").val();
        CoCd = $("#ddlCompany").val();
        SaleGrpId = localStorage.salesgrouprowid ;
        EmpId = -1;
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            ...
            $.ajax({
                ...
                url: apiurl + 'api/SalesPersonOperation',
                type: 'POST',
                data: { p_mode: "getlist", RowId: -1, SaleGrpId: SaleGrpId, EmpId: EmpId, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },


                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    SalesPersonObject.do_populateData(obj);
                },
                error: function (err) {
    ...
    do_populateData: (obj) => {
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                ...
                if (!SalesPersonObject._deleteperm[0]) {
        ...
        if (!SalesPersonObject._createperm[0]) {
        ...
        //if (!SalesPersonObject._accountmapvieweperm[0]) {
    ...
    do_getUserPagepermission: () => {
        MainObject.do_getuserpageaccess(SalesPersonObject);
        
        SalesPersonObject._vieweperm = MainObject.do_IsActionMenuPermission(SalesPersonObject.access, 'ADD SALES PERSON/REPRESENTATIVE', 'view');
        SalesPersonObject._createperm = MainObject.do_IsActionMenuPermission(SalesPersonObject.access, 'ADD SALES PERSON/REPRESENTATIVE', 'create');
        SalesPersonObject._editperm = MainObject.do_IsActionMenuPermission(SalesPersonObject.access, 'ADD SALES PERSON/REPRESENTATIVE', 'edit');
        SalesPersonObject._deleteperm = MainObject.do_IsActionMenuPermission(SalesPersonObject.access, 'ADD SALES PERSON/REPRESENTATIVE', 'delete');
        

    },
  
    do_populateUserDropdown: () => {
        
        $.ajax({
            url: apiurl + 'api/getEmployeeforSalesperson',
            type: 'POST',
            ...
            success: function (response) {
                ...
                for (var i = 0; i < response.length; i++) {
                    _html.push(
                        "<option value='" + response[i].RowId + "'>" + response[i].PNO + "</option>"
                    );
                }
                $("#ddPerson").html(_html.join(""));  
                SalesPersonObject.do_loaddata();
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                SalesPersonObject.rowid = response[0].RowId;
    ...
    do_loadwarehouselocation: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", LocationCd: '', CoCd: $("#ddlCompany").val(), LocationDesc: '', WhId: SalesPersonObject.rowid, AisleNo: -1, RackNo: -1, SelfNo: -1, BinNo: "-1", created_by: $("#txt").val(), RowId: -1 },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    SalesPersonObject.do_loadwarehouselocationData(obj);
                },
                error: function () {
...
var savedata = function () {
    ...
    if ($('#ddPerson').val() <1) {
        validate = false;
        gotfocus($("#ddPerson"));
        $.alertable.alert(`Sales Person/Representative Code required.`);
        //$("#txtCode").focus();
        return false;
    }
...
function saveFinal(imgId) {
    ...
    if (parseInt(SalesPersonObject.rowid) > 0) {
        $.ajax({

            url: apiurl + 'api/SalesPersonOperation',
            type: 'POST',
            data: { p_mode: "update", RowId: parseInt(SalesPersonObject.rowid), SaleGrpId: SaleGrpId, EmpId: EmpId, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
                ...
                if (response[0].msg == "true") {
                    ...
                    $.alertable.alert(`Data updated successfully.`, ``, `Ok`, ``).then(function () {
                        window.location = "salesperson.aspx";
                    });
                }
                else {
        ...
        $.ajax({
            ...
            url: apiurl + 'api/SalesPersonOperation',
            type: 'POST',
            data: { p_mode: "create", RowId: -1, SaleGrpId: SaleGrpId, EmpId: EmpId, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
...
var otherWindow = function (countryCd, mode) {
    ...
    if (localStorage.budgetgrouprowid) {
        if (mode == "map") {
            ...
            localStorage.menu_id_premission = SalesPersonObject._accountmapmenuid[1];
            localStorage.clickedmenu_id = SalesPersonObject._accountmapmenuid[1];
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == 'add') {
        companylogo = "";
        SalesPersonObject.rowid = '-1';
        showmodal();
        $('.modal-title').html('Sales Person/Representative - New');
    ...
    if (mode == 'edit') {
        ...
        $('.modal-title').html('Sales Person/Representative - Edit');
        $('#cbBlock').show();
        datablank();
        if (!SalesPersonObject._deleteperm[0]) {
        ...
        $('#txtCode').prop("disabled", true);
        SalesPersonObject.rowid = rowId;
        SalesPersonObject.do_loaddataedit(rowId);
        
    }
    else if(mode == 'view') {
        showmodal();
        datablank();
        $('.modal-title').html('Sales Person/Representative - View');
        $('#cbBlock').show();
       // $('#btnDelete').show();
        // $('#btnEdit').show();
        if (!SalesPersonObject._deleteperm[0]) {
        ...
        if (!SalesPersonObject._editperm[0]) {
            ...
            .then(
                function () {
                    ...
                    $.ajax({
                        ...
                        url: apiurl + 'api/SalesPersonOperation',
                        type: 'POST',
                        data: { p_mode: "delete", RowId: rowId, SaleGrpId: SaleGrpId, EmpId: EmpId, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
...
function PopulateState() {
    SalesPersonObject.do_populateLocationDropdown('STATE', $('#ddPincodeCountry').val(), $('#ddPincodeCounty'));
}
function PopulateCity() {
    SalesPersonObject.do_populateLocationDropdown('CITY', $('#ddPincodeCounty').val(), $('#ddPincodeCity'));
}
