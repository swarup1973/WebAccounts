...
$(document).ready(function () {
    ...
    $("#FAGroupDesc").html(localStorage.faitemgroup);
    //setImage();
    $("#myModal").modal({
        show: false,
        backdrop: 'static'
    ...
    //alert(localStorage.faitemgroupdesc);
    $("#FAGroupDesc").html(localStorage.faitemgroupdesc);
    
    
    ViewFixEdAssetGroupObject.do_loaddata();
    ViewFixEdAssetGroupObject.do_getUserPagepermission();
...
var ViewFixEdAssetGroupObject = {
    ...
    do_loaddata: () => {
        ...
        GroupId = localStorage.fagrouprowid;
      
        var _data;
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),

            $.ajax({
                url: apiurl + 'api/getFixedAssetFixedAssetMasterbyFaGroup',
                type: 'POST',
                data: { FAGrpId: GroupId},

                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                  
                    ViewFixEdAssetGroupObject.do_populateData(obj);
                },
                error: function (err) {
    ...
    do_populateData: (obj) => {
        ...
        var editor = new $.fn.dataTable.Editor({
            ...
            fields: [
                ...
                { label: "FASubType1", name: "FASubType1" },
                { label: "FABookType", name: "FABookType" },
                { label: "DepnMethod", name: "DepnMethod" },
        ...
        roletable.dataTable({
            ...
            buttons: [
                {
                    add: "create", text: 'New', className: "buttonsToHide"
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                ...
                if (!ViewFixEdAssetGroupObject._deleteperm[0]) {
        ...
        if (!ViewFixEdAssetGroupObject._createperm[0]) {
        ...
        if (!ViewFixEdAssetGroupObject._editperm[0]) {
        ...
        if (!ViewFixEdAssetGroupObject._vieweperm[0]) {
        ...
        if (!ViewFixEdAssetGroupObject._accountmapvieweperm[0]) {
    ...
    do_getUserPagepermission: () => {
        MainObject.do_getuserpageaccess(ViewFixEdAssetGroupObject);
        
        ViewFixEdAssetGroupObject._vieweperm = MainObject.do_IsActionMenuPermission(ViewFixEdAssetGroupObject.access, 'ITEM GROUP', 'view');
        ViewFixEdAssetGroupObject._createperm = MainObject.do_IsActionMenuPermission(ViewFixEdAssetGroupObject.access, 'ITEM GROUP', 'create');
        ViewFixEdAssetGroupObject._editperm = MainObject.do_IsActionMenuPermission(ViewFixEdAssetGroupObject.access, 'ITEM GROUP', 'edit');
        ViewFixEdAssetGroupObject._deleteperm = MainObject.do_IsActionMenuPermission(ViewFixEdAssetGroupObject.access, 'ITEM GROUP', 'delete');

        ViewFixEdAssetGroupObject._accountmapvieweperm = MainObject.do_IsActionMenuPermission(ViewFixEdAssetGroupObject.access, 'VIEW ITEMS UNDER GROUP', 'view');
        ViewFixEdAssetGroupObject._accountmapmenuid = MainObject.do_IsActionMenuPermission(ViewFixEdAssetGroupObject.access, 'VIEW ITEMS UNDER GROUP', 'menuid');
    ...
    do_populateMasterDropdown: () => {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { p_mode: "masternosequence", RowId: -1, GroupCd: GroupCd, CoCd: CoCd, GroupDesc: GroupDesc, ItemGroupNo: ItemGroupNo, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
                objVendor = response;
                var _html = [];
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            ...
            url: apiurl + 'api/InventoryItemGroupoperation',
            type: 'POST',
            data: { p_mode: "edit", RowId: id, GroupCd: GroupCd, CoCd: CoCd, GroupDesc: GroupDesc, ItemGroupNo: ItemGroupNo, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            ...
            success: function (response) {
                ...
                ViewFixEdAssetGroupObject.rowid = response[0].RowId;
                $("#txtCode").val(response[0].GroupCd);
                $("#txtDesc").val(response[0].GroupDesc);
                $("#ddSeqCode").val(response[0].ItemGroupNo);
    ...
    do_loadwarehouselocation: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", LocationCd: '', CoCd: $("#ddlCompany").val(), LocationDesc: '', WhId: ViewFixEdAssetGroupObject.rowid, AisleNo: -1, RackNo: -1, SelfNo: -1, BinNo: "-1", created_by: $("#txt").val(), RowId: -1 },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    ViewFixEdAssetGroupObject.do_loadwarehouselocationData(obj);
                },
                error: function () {
...
function saveFinal(imgId) {
    ...
    ItemGroupNo = $("#ddSeqCode").val();


    
    
    if (parseInt(ViewFixEdAssetGroupObject.rowid) > 0) {
        $.ajax({

            url: apiurl + 'api/InventoryItemGroupoperation',
            type: 'POST',
            data: { p_mode: "update", RowId: parseInt(ViewFixEdAssetGroupObject.rowid), GroupCd: GroupCd, CoCd: CoCd, GroupDesc: GroupDesc, ItemGroupNo: ItemGroupNo, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
                ...
                if (response[0].msg == "true") {
                    ...
                    $.alertable.alert(`Data updated successfully.`, ``, `Ok`, ``).then(function () {
                        window.location = "item-group.aspx";
                    });
                }
                else {
        ...
        $.ajax({
            ...
            url: apiurl + 'api/InventoryItemGroupoperation',
            type: 'POST',
            data: { p_mode: "create", RowId: -1, GroupCd: GroupCd, CoCd: CoCd, GroupDesc: GroupDesc, ItemGroupNo: ItemGroupNo, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
...
var otherWindow = function (countryCd, mode) {
    ...
    localStorage.itemgrouprowid = countryCd;
    //$('#addressbook').DataTable().rows('.selected').data()[0].AddressCode
    if (localStorage.budgetgrouprowid) {
        if (mode == "viewitem") {

            localStorage.menu_id_premission = ViewFixEdAssetGroupObject._accountmapmenuid[1];
            localStorage.clickedmenu_id = ViewFixEdAssetGroupObject._accountmapmenuid[1];
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == "viewlocation") {
        ViewFixEdAssetGroupObject.rowid = rowId;
        showmodalview();

    }
    if (mode == 'add') {
        companylogo = "";
        ViewFixEdAssetGroupObject.rowid = '-1';
        showmodal();
        $('.modal-title').html('Item Group - New');
    ...
    if (mode == 'edit') {
        ...
        $('.modal-title').html('Item Group - Edit');
        $('#cbBlock').show();
        datablank();
        if (!ViewFixEdAssetGroupObject._deleteperm[0]) {
        ...
        $('#txtCode').prop("disabled", true);
        ViewFixEdAssetGroupObject.rowid = rowId;
        ViewFixEdAssetGroupObject.do_loaddataedit(rowId);
...
function PopulateState() {
    ViewFixEdAssetGroupObject.do_populateLocationDropdown('STATE', $('#ddPincodeCountry').val(), $('#ddPincodeCounty'));
}
function PopulateCity() {
    ViewFixEdAssetGroupObject.do_populateLocationDropdown('CITY', $('#ddPincodeCounty').val(), $('#ddPincodeCity'));
}
