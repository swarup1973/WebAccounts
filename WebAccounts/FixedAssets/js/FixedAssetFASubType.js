...
$(document).ready(function () {
    ...
    FixedAssetFASubTypeObject.do_populateMasterDropdown();

    
    //FixedAssetFASubTypeObject.do_loaddata();
    FixedAssetFASubTypeObject.do_getUserPagepermission();
...
var FixedAssetFASubTypeObject = {
    ...
    do_loaddata: () => {
        ...
        var RowId, SubTypeCd, CoCd, FASubType, FATypeId, IsBlock, created_by, creator_MAC_add;
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            ...
            $.ajax({
                url: apiurl + 'api/FixedAssetFASubTypeoperation',
                type: 'POST',
                data: { p_mode: "getlist", RowId: -1, SubTypeCd: SubTypeCd, CoCd: CoCd, FASubType: FASubType, FATypeId: FATypeId, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },

                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                  
                    FixedAssetFASubTypeObject.do_populateData(obj);
                },
                error: function (err) {
    ...
    do_populateData: (obj) => {
        ...
        var editor = new $.fn.dataTable.Editor({
            table: "#addressbook",
            fields: [
                { label: "SubTypeCd", name: "SubTypeCd" },
                { label: "FASubType", name: "FASubType" },
                { label: "FAType", name: "FAType" }
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                ...
                if (!FixedAssetFASubTypeObject._deleteperm[0]) {
        ...
        if (!FixedAssetFASubTypeObject._createperm[0]) {
        ...
        if (!FixedAssetFASubTypeObject._editperm[0]) {
        ...
        if (!FixedAssetFASubTypeObject._vieweperm[0]) {
    ...
    do_getUserPagepermission: () => {
        MainObject.do_getuserpageaccess(FixedAssetFASubTypeObject);
        
        FixedAssetFASubTypeObject._vieweperm = MainObject.do_IsActionMenuPermission(FixedAssetFASubTypeObject.access, 'FIXED ASSETS SUB-TYPES', 'view');
        FixedAssetFASubTypeObject._createperm = MainObject.do_IsActionMenuPermission(FixedAssetFASubTypeObject.access, 'FIXED ASSETS SUB-TYPES', 'create');
        FixedAssetFASubTypeObject._editperm = MainObject.do_IsActionMenuPermission(FixedAssetFASubTypeObject.access, 'FIXED ASSETS SUB-TYPES', 'edit');
        FixedAssetFASubTypeObject._deleteperm = MainObject.do_IsActionMenuPermission(FixedAssetFASubTypeObject.access, 'FIXED ASSETS SUB-TYPES', 'delete');
    ...
    do_populateMasterDropdown: () => {
        ...
        $.ajax({
            ...
            url: apiurl + 'api/FixedAssetFASubTypeoperation',
            type: 'POST',
            data: { p_mode: "mastertype", RowId: -1, SubTypeCd: SubTypeCd, CoCd: CoCd, FASubType: FASubType, FATypeId: FATypeId, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            ...
            success: function (response) {
                ...
                for (var i = 0; i < response.length; i++) {
                    _html.push(
                        "<option value='" + response[i].RowId + "'>" + response[i].FAType + "</option>"
                    );
                }
                $("#ddFaType").html(_html.join(""));  
                FixedAssetFASubTypeObject.do_loaddata();
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            ...
            url: apiurl + 'api/FixedAssetFASubTypeoperation',
            type: 'POST',
            data: { p_mode: "edit", RowId: id, SubTypeCd: SubTypeCd, CoCd: CoCd, FASubType: FASubType, FATypeId: FATypeId, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            ...
            success: function (response) {
                ...
                FixedAssetFASubTypeObject.rowid = response[0].RowId;
                $("#txtCode").val(response[0].SubTypeCd);
                $("#txtDesc").val(response[0].FASubType);
                $("#ddFaType").val(response[0].FATypeId);
    ...
    do_loadwarehouselocation: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", LocationCd: '', CoCd: $("#ddlCompany").val(), LocationDesc: '', WhId: FixedAssetFASubTypeObject.rowid, AisleNo: -1, RackNo: -1, SelfNo: -1, BinNo: "-1", created_by: $("#txt").val(), RowId: -1 },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    FixedAssetFASubTypeObject.do_loadwarehouselocationData(obj);
                },
                error: function () {
...
function saveFinal(imgId) {
    ...
    created_by = $("#txt").val();
    CoCd = $("#ddlCompany").val();
    SubTypeCd = $("#txtCode").val();
    FASubType = $("#txtDesc").val();
    FATypeId = $("#ddFaType").val();

    
    
    if (parseInt(FixedAssetFASubTypeObject.rowid) > 0) {
        $.ajax({

            url: apiurl + 'api/FixedAssetFASubTypeoperation',
            type: 'POST',
            data: { p_mode: "update", RowId: parseInt(FixedAssetFASubTypeObject.rowid), SubTypeCd: SubTypeCd, CoCd: CoCd, FASubType: FASubType, FATypeId: FATypeId, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            ...
            success: function (response) {
                ...
                if (response[0].msg == "true") {
                    ...
                    $.alertable.alert(`Data updated successfully.`, ``, `Ok`, ``).then(function () {
                        window.location = "fixed-assets-sub-type-setup.aspx";
                    });
                }
                else {
        ...
        $.ajax({
            url: apiurl + 'api/FixedAssetFASubTypeoperation',
            type: 'POST',
            data: { p_mode: "create", RowId: -1, SubTypeCd: SubTypeCd, CoCd: CoCd, FASubType: FASubType, FATypeId: FATypeId, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
...
var otherWindow = function (countryCd, mode) {
    ...
    if (localStorage.fagrouprowid) {
        if (mode == "faitem") {
            ...
            localStorage.menu_id_premission = FixedAssetFASubTypeObject._accountmapmenuid[1];
            localStorage.clickedmenu_id = FixedAssetFASubTypeObject._accountmapmenuid[1];
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == "viewlocation") {
        FixedAssetFASubTypeObject.rowid = rowId;
        showmodalview();

    }
    if (mode == 'add') {
        companylogo = "";
        FixedAssetFASubTypeObject.rowid = '-1';
        showmodal();
        $('.modal-title').html('Fixed Assets Sub-type Setup - New');
    ...
    if (mode == 'edit') {
        ...
        $('.modal-title').html('Fixed Assets Sub-type Setup - Edit');
        $('#cbBlock').show();
        datablank();
        if (!FixedAssetFASubTypeObject._deleteperm[0]) {
        ...
        $('#txtCode').prop("disabled", true);
        FixedAssetFASubTypeObject.rowid = rowId;
        FixedAssetFASubTypeObject.do_loaddataedit(rowId);
        
    }
    else if(mode == 'view') {
        showmodal();
        datablank();
        $('.modal-title').html('Fixed Assets Sub-type Setup - View');
        $('#cbBlock').show();
       // $('#btnDelete').show();
        // $('#btnEdit').show();
        if (!FixedAssetFASubTypeObject._deleteperm[0]) {
            ...
            .then(
                function () {
                    ...
                    $.ajax({
                        ...
                        url: apiurl + 'api/FixedAssetFASubTypeoperation',
                        type: 'POST',
                        data: { p_mode: "delete", RowId: rowId, SubTypeCd: SubTypeCd, CoCd: CoCd, FASubType: FASubType, FATypeId: FATypeId, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
...
function PopulateState() {
    FixedAssetFASubTypeObject.do_populateLocationDropdown('STATE', $('#ddPincodeCountry').val(), $('#ddPincodeCounty'));
}
function PopulateCity() {
    FixedAssetFASubTypeObject.do_populateLocationDropdown('CITY', $('#ddPincodeCounty').val(), $('#ddPincodeCity'));
}
