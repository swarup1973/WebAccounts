$(document).ready(function () {
    DimSetup.cocd = $('#ddlCompany').val();
    DimSetup.dimsetupdata.cocd = $('#ddlCompany').val();
    ...
    if (!DimSetup._createperm[0]) {
        ...
        $('#btn_save').attr('title', 'do not have permission to Add New Record!!!');
        ($('#btn_save').children()).removeAttr("onclick");
        $('#btn_save').hide();
    }
...
var DimSetup = {
    ...
    do_getUserPagepermission: () => {
        ...
        DimSetup.access = (DimSetup.access).replace('\r\n', '');

        DimSetup._vieweperm = MainObject.do_IsActionMenuPermission(DimSetup.access, 'ASSETS DIMENSION', 'view');
        DimSetup._createperm = MainObject.do_IsActionMenuPermission(DimSetup.access, 'ASSETS DIMENSION', 'create');
        DimSetup._editperm = MainObject.do_IsActionMenuPermission(DimSetup.access, 'ASSETS DIMENSION', 'edit');
        DimSetup._deleteperm = MainObject.do_IsActionMenuPermission(DimSetup.access, 'ASSETS DIMENSION', 'delete');
        DimSetup._mainmenuid = MainObject.do_IsActionMenuPermission(DimSetup.access, 'ASSETS DIMENSION', 'menuid');
    },
        ...
        do_loadlookup: () => {
            if (localStorage.DimSetup_lookup == undefined || localStorage.DimSetup_lookup == "undefined") {
                ...
                $.ajax({
                    type: "POST",
                    async: false,
                    //url: "../GeneralLedger/chartofacct.aspx/loadlookupdata",
                    url: "../handler/datahandler.aspx/loadDimensionVendorCustomer",
                    ...
                    success: function (result) {
                        ...
                        for (var i = 0; i < obj.length; i++) {
                            ...
                            for (var key in objnew) {
                                ...
                                if (attrName.toLowerCase() == "table") {//dimension_table
                                    DimSetup.lookup.dimension_table = objnew[key];
                                }
                                if (attrName.toLowerCase() == "table1") {//dimensionValue
                                    DimSetup.lookup.dimensionValue = objnew[key];
                                }
                                if (attrName.toLowerCase() == "table2") {//dimensionset
    ...
    do_loaddimSetupData: () => {
        ...
        if (queryString('id') != undefined || queryString("id") != null) {
            ...
            window.location.href = "fixed-assets-master.aspx";
        }

        var _data = '{rowid: "' + DimSetup.dimsetupdata.rowid + '", cocd:"' + DimSetup.cocd + '"}';

        $.ajax({
            type: "POST",
            url: "assets-dimension.aspx/doloadDimSetupData",
            data: _data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
    ...
    dosaveDimSetup: () => {
        ...
        if (validate == true) {
            ...
            var _url = "assets-dimension.aspx/doSavedimsetup";
            $.ajax({
                type: "POST",
                url: _url,
                data: JSON.stringify(_passdata),
