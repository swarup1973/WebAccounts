...
$(document).ready(function () {
    ...
    ItemmasterOverviewObject.do_checkcodeentry("Procurement_Item");
...
var ItemmasterOverviewObject = {
    ...
    _viewitempictureperm: '',
    _viewitempicturemenuid: '',
    _viewnotesperm: '',
    _viewnotesmenuid: '',
    _stopperm: '',
    _blockperm: '',
    ...
    do_populatewarehouseData: (obj) => {
        ...
        var editor = new $.fn.dataTable.Editor({
            ...
            fields: [
                ...
                { label: "ItemTypeVal", name: "ItemTypeVal" }
        ...
        roletable.dataTable({
            ...
            buttons: [
                ...
                {
                    add: "viewitemprice", text: 'View Item Picture', action: function () { roleaction($('.selected').attr('rowid'), 'ITEMPRICE'); },
                    attr: {
                ...
                {
                    add: "viewnotes", text: 'View Notes', editor: editor, action: function () { roleaction($('.selected').attr('rowid'), 'NOTES'); },
                    attr: {
                        title: 'View Notes',
                        id: 'view_notes'
                    }
                ...
                {
                    add: "ohb", text: 'On Hand', editor: editor, action: function () { roleaction($('.selected').attr('rowid'), 'view'); },
                    attr: {
                        title: 'Transactions',
                        id: 'ohb'
                    }
        ...
        if (!ItemmasterOverviewObject._viewitempictureperm[0]) {
            $('#view_item_price').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#view_item_price').prop("disabled", true);
            $('#view_item_price').attr('title', 'do not have permission !!!');
            table.button(4).action(function () {
                this.active(false);
            });
        }
        if (!ItemmasterOverviewObject._viewnotesperm[0]) {
            $('#view_notes').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#view_notes').prop("disabled", true);
            $('#view_notes').attr('title', 'do not have permission !!!');
            table.button(5).action(function () {
                this.active(false);
            });
        }
        if (!ItemmasterOverviewObject._stopperm[0]) {
            $('#stop').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#stop').prop("disabled", true);
            $('#stop').attr('title', 'do not have permission !!!');
            table.button(7).action(function () {
                this.active(false);
            });
        }
        if (!ItemmasterOverviewObject._blockperm[0]) {
            $('#block').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#block').prop("disabled", true);
            $('#block').attr('title', 'do not have permission !!!');
            table.button(6).action(function () {
                this.active(false);
            });
        }
        if (!ItemmasterOverviewObject._transactionperm[0]) {
            $('#Transactions').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#Transactions').prop("disabled", true);
            $('#Transactions').attr('title', 'do not have permission !!!');
            table.button(19).action(function () {
                this.active(false);
            });
        }
        if (!ItemmasterOverviewObject._itemvendorpriceperm[0]) {
            ...
            $('#Item_Vendor_Price').attr('title', 'do not have permission !!!');
            table.button(11).action(function () {
                this.active(false);
            });
        }
        if (!ItemmasterOverviewObject._itemcustomerdiscperm[0]) {
            $('#Item-Customer-Discount').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#Item-Customer-Discount').prop("disabled", true);
            $('#Item-Customer-Discount').attr('title', 'do not have permission !!!');
            table.button(14).action(function () {
                this.active(false);
            });
        }
    ...
    do_getUserPagepermission: () => {
        ...
        ItemmasterOverviewObject._stopperm = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'STOP', 'view');
        ItemmasterOverviewObject._blockperm = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'BLOCK', 'view');
        ...
        ItemmasterOverviewObject._unitconversionperm = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'ITEM SPECIFIC CONVERSION', 'view');
        ItemmasterOverviewObject._unitconversionemenuid = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'ITEM SPECIFIC CONVERSION', 'menuid');
        ...
        ItemmasterOverviewObject._viewitempictureperm = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'VIEW ITEM PICTURE', 'view');
        ItemmasterOverviewObject._viewitempicturemenuid = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'VIEW ITEM PICTURE', 'menuid');
        ItemmasterOverviewObject._viewnotesperm = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'VIEW NOTES', 'view');
        ItemmasterOverviewObject._viewnotesmenuid = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'VIEW NOTES', 'menuid');
        ItemmasterOverviewObject._transactionperm = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'TRANSACTONS', 'view');
        ItemmasterOverviewObject._transactionmenuid = MainObject.do_IsActionMenuPermission(ItemmasterOverviewObject.access, 'TRANSACTONS', 'menuid');

        
    },
    fn_updatestopblock: (ctype,rowid) => {
        var issBlock=0,ItemCd = '', CoCd = $("#ddlCompany").val(), ItemDesc='', SearchDesc='', InternalId=0, GroupId=0, CategoryId=0, ItemType=0, PostingGrpId=0, IsStop=0, AllowNegtvStock=0, ItemPic=0, Notes='', SI_UomId=0, SI_STCompId=0
            , PR_LeadTime=0, PR_DefltVendorId=0, PR_STCompId=0, PR_UomId=0, PR_LossOnPurchase = 0, CP_StdCost = 0, CP_ProfitPer = 0, CP_SalePrice =0, CP_MaxSalePrice=0, CP_CostMethod=0
            , CP_CostPriceByVariant =0, P_MinStockLevel=0, P_SafetyStock=0, P_MaxStockLevel=0, P_StockWarning=0, TM_DirectInvoicing=0, TM_ACARN=0, TM_QuarantineReq=0
            , TM_RegdReq=0, TM_RecvReq=0, TM_PickReq=0, TM_ReserveReq=0, TM_Reservation=0, SA_DefWarehouse=0, SA_DefLocation=0, SA_UomId=0, SA_CaptureDOM=0, SA_BBU=0, SA_Warranty=0
        ...
        $.ajax({
            url: apiurl + 'api/procurementItemOperation',
                type: 'POST',
                data: { p_mode: ctype, RowId: rowid, ItemCd: ItemCd, CoCd: CoCd, ItemDesc: ItemDesc, SearchDesc: SearchDesc, InternalId: InternalId, GroupId: GroupId, CategoryId: CategoryId, ItemType: ItemType, PostingGrpId: PostingGrpId, IsBlock: issBlock, IsStop: IsStop, AllowNegtvStock: AllowNegtvStock, ItemPic: ItemPic, Notes: Notes, SI_UomId: SI_UomId, SI_STCompId: SI_STCompId, PR_LeadTime: PR_LeadTime, PR_DefltVendorId: PR_DefltVendorId, PR_STCompId: PR_STCompId, PR_UomId: PR_UomId, PR_LossOnPurchase: PR_LossOnPurchase, CP_StdCost: CP_StdCost, CP_ProfitPer: CP_ProfitPer, CP_SalePrice: CP_SalePrice, CP_MaxSalePrice: CP_MaxSalePrice, CP_CostMethod: CP_CostMethod, CP_CostPriceByVariant: CP_CostPriceByVariant, P_MinStockLevel: P_MinStockLevel, P_SafetyStock: P_SafetyStock, P_MaxStockLevel: P_MaxStockLevel, P_StockWarning: P_StockWarning, TM_DirectInvoicing: TM_DirectInvoicing, TM_ACARN: TM_ACARN, TM_QuarantineReq: TM_QuarantineReq, TM_RegdReq: TM_RegdReq, TM_RecvReq: TM_RecvReq, TM_PickReq: TM_PickReq, TM_ReserveReq: TM_ReserveReq, TM_Reservation: TM_Reservation, SA_DefWarehouse: SA_DefWarehouse, SA_DefLocation: SA_DefLocation, SA_UomId: SA_UomId, SA_CaptureDOM: SA_CaptureDOM, SA_BBU: SA_BBU, SA_Warranty: SA_Warranty, SA_WarrantyPeriod: SA_WarrantyPeriod, SA_TrackBatch: SA_TrackBatch, SA_TrackSrNo: SA_TrackSrNo, SA_LotNo: SA_LotNo, SA_VariantId_1: SA_VariantId_1, SA_VariantId_2: SA_VariantId_2, SA_VariantId_3: SA_VariantId_3, SA_VariantId_4: SA_VariantId_4, FT_GTIN: FT_GTIN, FT_TariffCd: FT_TariffCd, FT_COOId: FT_COOId, TI_HSNId: TI_HSNId, TI_PriceWithTax: TI_PriceWithTax, created_by: created_by, creator_MAC_add: creator_MAC_add },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
    ...
    do_loadnotesimage: (id,ctype) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                ItemmasterOverviewObject.rowid = response[0].RowId;
                if (ctype == 'NOTES') {
                    $("#pnotes").html(response[0].Notes);
                }
                if (ctype == 'IMAGE') {
                    try {
                        setImageP(ItemmasterOverviewObject.rowid);
                    }
                    catch (e) {
...
function checkvariantValue() {
    var rtn = 0;
    if (parseInt($('#ddVariantId_1').val()) > 0) {
        if ($('#ddVariantId_1').val() == $('#ddVariantId_2').val() || $('#ddVariantId_1').val() == $('#ddVariantId_3').val() || $('#ddVariantId_1').val() == $('#ddVariantId_4').val()) {
            rtn = 1;
        }
    }
    if (parseInt($('#ddVariantId_2').val()) > 0 && rtn !=1) {
        if ($('#ddVariantId_2').val() == $('#ddVariantId_3').val() || $('#ddVariantId_2').val() == $('#ddVariantId_4').val()) {
            rtn = 1;
        }
    }
    if (parseInt($('#ddVariantId_3').val()) > 0 && rtn != 1) {
        if ($('#ddVariantId_3').val() == $('#ddVariantId_4').val()) {
            rtn = 1;
        }
    }
    return rtn;
...
var savedata = function () {
    var validate = true;
    //
    if (manualentry == true && $('#txtItemCd').val().length < 1) {
        validate = false;
        $.alertable.alert(`Item No required.`);
        $("#txtItemCd").focus();
        return false;
    }
    else if (checkcode() != 0) {
        validate = false;
        $.alertable.alert(`Item No format mismatch / Invalid range.`);
        $("#txtItemCd").focus();
        return false;
    }
    else if ($('#txtItemDesc').val().length < 1) {
        validate = false;
        gotfocus($('#txtItemDesc'));
        //$('#txtItemDesc').animate({ left: 0, duration: 'slow' });
        //$("#txtItemDesc").focus();
    ...
    // }
        ...
        validate = false;
        $.alertable.alert(`Sales & Invoicing - Sale Unit of Measurement required.`);
        $("#ddUomId").focus();
        return false;
    }
    else if ($('#ddtaxgroupitem').val() < 1) {
        validate = false;
        $.alertable.alert(`Sales & Invoicing - Tax Group-Item required.`);
        $("#ddtaxgroupitem").focus();
        return false;
    }
    else if ($('#ddTaxItemGroup').val() < 1) {
        validate = false;
        $.alertable.alert(`Purchase & Replenishment - Tax Group-Item required.`);
        $("#ddTaxItemGroup").focus();
        return false;
    }
    else if ($('#ddStockUOM').val() < 1) {
        validate = false;
        $.alertable.alert(`Storage & Administration - Stock Unit of Measurement required.`);
        $("#ddStockUOM").focus();
        return false;
    }
    else if ($('#chkCostPriceByVariant').is(':checked') && $('#ddVariantId_1').val() < 1 && $('#ddVariantId_2').val() < 1 && $('#ddVariantId_3').val() < 1 && $('#ddVariantId_4').val() < 1) {
        validate = false;
        $.alertable.alert(`atleast one variant code required.`);
        $('#ddVariantId_1').focus();
        return false;


    }
    else if ($('#chkCostPriceByVariant').is(':checked') && checkvariantValue() == 1) {
        validate = false;
        $.alertable.alert(`Duplicate variant code.`);
        $('#ddVariantId_1').focus();
        return false;
    }

    else if ($('#chkReserveReq').is(':checked') && $('#ddReservation').val() < 1) {
        validate = false;
        $.alertable.alert(`Reservation required.`);
        $("#ddReservation").focus();
        return false;
    }
    else if ($('#chkWarranty').is(':checked') && $('#txtWarrantyPeriod').val() < 1) {
        validate = false;
        $.alertable.alert(`Enter Warranty Period.`);
        $("#txtWarrantyPeriod").focus();
        return false;
    }

    else if ($('#ddDefWarehouse').val() > 0 && $('#ddDefLocation').val() < 1) {

        validate = false;
        $.alertable.alert(`Default Location required.`);
        $("#ddDefLocation").focus();
        return false;


    }
    else if ($('#ddPurchaseUOM').val() < 1) {
        validate = false;
        $.alertable.alert(`Purchase & Replenishment - Purchase Unit of Measurement required.`);
        $("#ddPurchaseUOM").focus();
        return false;
    }
    else if ($('#ddCostMethod').val() < 1) {
        validate = false;
        $.alertable.alert(`Cost & Price - Costing Method required.`);
        $("#ddCostMethod").focus();
        return false;
    }
...
var savedata_old = function () {
    ...
    // }
        ...
        validate = false;
        $.alertable.alert(`Purchase Unit of Measurement required.`);
        $("#ddPurchaseUOM").focus();
        return false;
    }
    else if ($('#ddCostMethod').val() < 1) {
        validate = false;
        $.alertable.alert(`Costing Method required.`);
        $("#ddCostMethod").focus();
        return false;
    }
...
var otherWindow = function (countryCd, mode) {
    ...
    if (localStorage.itemmasteroverviewrowid) {
        if (mode == "substitute") {
            ...
            localStorage.itemmasteroverviewitemno = $('#addressbook').DataTable().rows('.selected').data()[0].ItemCd;
            localStorage.itemmasteroverviewitemdesc = $('#addressbook').DataTable().rows('.selected').data()[0].ItemDesc;
            localStorage.itemmasteroverviewitemtype = $('#addressbook').DataTable().rows('.selected').data()[0].ItemTypeVal;
            //localStorage.AddressDetailCountryName = $('#addressbook').DataTable().rows('.selected').data()[0].CountryName;
            //localStorage.AddressDetailCountryCode = $('#addressbook').DataTable().rows('.selected').data()[0].CountryCd;
    ...
    if (localStorage.itemmasteroverviewrowid) {
        if (mode == "UNITCONVERSION") {
            ...
            localStorage.clickedmenu_id = ItemmasterOverviewObject._unitconversionemenuid[1];
            localStorage.menu_id_premission = ItemmasterOverviewObject._unitconversionemenuid[1];
            localStorage.itemmasteroverviewitemno = $('#addressbook').DataTable().rows('.selected').data()[0].ItemCd;
            localStorage.itemmasteroverviewitemdesc = $('#addressbook').DataTable().rows('.selected').data()[0].ItemDesc;
            //localStorage.AddressDetailCountryName = $('#addressbook').DataTable().rows('.selected').data()[0].CountryName;
            //localStorage.AddressDetailCountryCode = $('#addressbook').DataTable().rows('.selected').data()[0].CountryCd;
    ...
    if (localStorage.itemmasteroverviewrowid) {
        if (mode == "ITEMVENDORPRICE") {
            ...
            localStorage.clickedmenu_id = ItemmasterOverviewObject._itemvendorpricemenuid[1];
            localStorage.menu_id_premission = ItemmasterOverviewObject._itemvendorpricemenuid[1];
            localStorage.itemmasteroverviewitemno = $('#addressbook').DataTable().rows('.selected').data()[0].ItemCd;
            localStorage.itemmasteroverviewitemdesc = $('#addressbook').DataTable().rows('.selected').data()[0].ItemDesc;
            //localStorage.AddressDetailCountryName = $('#addressbook').DataTable().rows('.selected').data()[0].CountryName;
            //localStorage.AddressDetailCountryCode = $('#addressbook').DataTable().rows('.selected').data()[0].CountryCd;
    ...
    if (localStorage.itemmasteroverviewrowid) {
        if (mode == "ITEMCUSTOMERPRICE") {
            ...
            localStorage.clickedmenu_id = ItemmasterOverviewObject._itemcustomerpricemenuid[1];
            localStorage.menu_id_premission = ItemmasterOverviewObject._itemcustomerpricemenuid[1];
            localStorage.itemmasteroverviewitemno = $('#addressbook').DataTable().rows('.selected').data()[0].ItemCd;
            localStorage.itemmasteroverviewitemdesc = $('#addressbook').DataTable().rows('.selected').data()[0].ItemDesc;
            //localStorage.AddressDetailCountryName = $('#addressbook').DataTable().rows('.selected').data()[0].CountryName;
            //localStorage.AddressDetailCountryCode = $('#addressbook').DataTable().rows('.selected').data()[0].CountryCd;
    ...
    if (localStorage.itemmasteroverviewrowid) {
        if (mode == "ITEMCUSTOMERDISCOUNT") {
            ...
            localStorage.clickedmenu_id = ItemmasterOverviewObject._itemcustomerdismenuid[1];
            localStorage.menu_id_premission = ItemmasterOverviewObject._itemcustomerdismenuid[1];
            localStorage.itemmasteroverviewitemno = $('#addressbook').DataTable().rows('.selected').data()[0].ItemCd;
            localStorage.itemmasteroverviewitemdesc = $('#addressbook').DataTable().rows('.selected').data()[0].ItemDesc;
            //localStorage.AddressDetailCountryName = $('#addressbook').DataTable().rows('.selected').data()[0].CountryName;
            //localStorage.AddressDetailCountryCode = $('#addressbook').DataTable().rows('.selected').data()[0].CountryCd;
    ...
    if (localStorage.itemmasteroverviewrowid) {
        if (mode == "PURCHASEINVOICEDISCOUNT") {
            ...
            localStorage.clickedmenu_id = ItemmasterOverviewObject._purchaseinvoicediscmenuid[1];
            localStorage.menu_id_premission = ItemmasterOverviewObject._purchaseinvoicediscmenuid[1];
            localStorage.itemmasteroverviewitemno = $('#addressbook').DataTable().rows('.selected').data()[0].ItemCd;
            localStorage.itemmasteroverviewitemdesc = $('#addressbook').DataTable().rows('.selected').data()[0].ItemDesc;
            //localStorage.AddressDetailCountryName = $('#addressbook').DataTable().rows('.selected').data()[0].CountryName;
            //localStorage.AddressDetailCountryCode = $('#addressbook').DataTable().rows('.selected').data()[0].CountryCd;
    ...
    if (localStorage.itemmasteroverviewrowid) {
        if (mode == "SALEINVOICEDISCOUNT") {
            ...
            localStorage.clickedmenu_id = ItemmasterOverviewObject._saleinvoicediscmenuid[1];
            localStorage.menu_id_premission = ItemmasterOverviewObject._saleinvoicediscmenuid[1];
    ...
    if (localStorage.itemmasteroverviewrowid) {
        if (mode == "ITEMVENDORDISCOUNT") {
            ...
            localStorage.clickedmenu_id = ItemmasterOverviewObject._itemvendordiscmenuid[1];
            localStorage.menu_id_premission = ItemmasterOverviewObject._itemvendordiscmenuid[1];
            localStorage.itemmasteroverviewitemno = $('#addressbook').DataTable().rows('.selected').data()[0].ItemCd;
            localStorage.itemmasteroverviewitemdesc = $('#addressbook').DataTable().rows('.selected').data()[0].ItemDesc;
            //localStorage.AddressDetailCountryName = $('#addressbook').DataTable().rows('.selected').data()[0].CountryName;
            //localStorage.AddressDetailCountryCode = $('#addressbook').DataTable().rows('.selected').data()[0].CountryCd;
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == "NOTES") {
        ItemmasterOverviewObject.rowid = rowId;
        showmodalnotes();
        ItemmasterOverviewObject.do_loadnotesimage(ItemmasterOverviewObject.rowid, 'NOTES');

    }
    if (mode == "ITEMPRICE") {
        ItemmasterOverviewObject.rowid = rowId;
        showmodalpic();
        ItemmasterOverviewObject.do_loadnotesimage(ItemmasterOverviewObject.rowid, 'IMAGE');
    ...
    if (mode == 'add') {
        ...
        if (manualentry == true) {
            $('#txtItemCd').attr("disabled", false);
        }
        if (manualentry == false) {
            $('#txtItemCd').attr("disabled", true);
        }
    ...
    if (mode == 'edit') {
        ...
        var ccStop = $('#addressbook').DataTable().rows('.selected').data()[0].IsStop;
        if (ccStop == 'checked') {
            $('#btnSave').hide();
        }
        $('#txtItemCd').attr("disabled", true);
        ...
        $.alertable.custconfirm(`Are you want to block the Record ?`, ``, `Yes`, `No`)
            .then(
                function () {
                    var _data;
                    _data = '{rowid:"' + rowId + '"}';
                    ItemmasterOverviewObject.fn_updatestopblock('block',rowId)
        ...
        $.alertable.custconfirm(`Are you want to stop the Record ?`, ``, `Yes`, `No`)
            .then(
                function () {
                    var _data;
                    _data = '{rowid:"' + rowId + '"}';
                    ItemmasterOverviewObject.fn_updatestopblock('stop',rowId)
            ...
            .then(
                function () {
                    ...
                    $.ajax({
                        url: apiurl + 'api/procurementItemOperation',
                        type: 'POST',
                        data: { p_mode: "remov", RowId: rowId, ItemCd: ItemCd, CoCd: CoCd, ItemDesc: ItemDesc, SearchDesc: SearchDesc, InternalId: InternalId, GroupId: GroupId, CategoryId: CategoryId, ItemType: ItemType, PostingGrpId: PostingGrpId, IsBlock: issBlock, IsStop: IsStop, AllowNegtvStock: AllowNegtvStock, ItemPic: ItemPic, Notes: Notes, SI_UomId: SI_UomId, SI_STCompId: SI_STCompId, PR_LeadTime: PR_LeadTime, PR_DefltVendorId: PR_DefltVendorId, PR_STCompId: PR_STCompId, PR_UomId: PR_UomId, PR_LossOnPurchase: PR_LossOnPurchase, CP_StdCost: CP_StdCost, CP_ProfitPer: CP_ProfitPer, CP_SalePrice: CP_SalePrice, CP_MaxSalePrice: CP_MaxSalePrice, CP_CostMethod: CP_CostMethod, CP_CostPriceByVariant: CP_CostPriceByVariant, P_MinStockLevel: P_MinStockLevel, P_SafetyStock: P_SafetyStock, P_MaxStockLevel: P_MaxStockLevel, P_StockWarning: P_StockWarning, TM_DirectInvoicing: TM_DirectInvoicing, TM_ACARN: TM_ACARN, TM_QuarantineReq: TM_QuarantineReq, TM_RegdReq: TM_RegdReq, TM_RecvReq: TM_RecvReq, TM_PickReq: TM_PickReq, TM_ReserveReq: TM_ReserveReq, TM_Reservation: TM_Reservation, SA_DefWarehouse: SA_DefWarehouse, SA_DefLocation: SA_DefLocation, SA_UomId: SA_UomId, SA_CaptureDOM: SA_CaptureDOM, SA_BBU: SA_BBU, SA_Warranty: SA_Warranty, SA_WarrantyPeriod: SA_WarrantyPeriod, SA_TrackBatch: SA_TrackBatch, SA_TrackSrNo: SA_TrackSrNo, SA_LotNo: SA_LotNo, SA_VariantId_1: SA_VariantId_1, SA_VariantId_2: SA_VariantId_2, SA_VariantId_3: SA_VariantId_3, SA_VariantId_4: SA_VariantId_4, FT_GTIN: FT_GTIN, FT_TariffCd: FT_TariffCd, FT_COOId: FT_COOId, TI_HSNId: TI_HSNId, TI_PriceWithTax: TI_PriceWithTax, created_by: created_by, creator_MAC_add: creator_MAC_add },
                        contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                        success: function (response) {
                            console.log(response);
                            if (response[0].msg == "true") {
