...
$(document).ready(function () {
    ...
    $("#GroupDesc").html(localStorage.GroupDesc);
    
    ViewItemGroupObject.do_loaddata();
    ViewItemGroupObject.do_getUserPagepermission();
...
var ViewItemGroupObject = {
    ...
    do_loaddata: () => {
        ...
        GroupId = localStorage.itemgrouprowid;

        var _data;
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),

            $.ajax({
                url: apiurl + 'api/GetProcurementItemByGroup',
                type: 'POST',
                ...
                success: function (response) {
                    ...
                    ViewItemGroupObject.do_populateData(obj);
                },
                error: function (err) {
    ...
    do_populateData: (obj) => {
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                ...
                if (!ViewItemGroupObject._deleteperm[0]) {
        ...
        if (!ViewItemGroupObject._createperm[0]) {
        ...
        if (!ViewItemGroupObject._editperm[0]) {
        ...
        if (!ViewItemGroupObject._vieweperm[0]) {
        ...
        if (!ViewItemGroupObject._accountmapvieweperm[0]) {
    ...
    do_getUserPagepermission: () => {
        MainObject.do_getuserpageaccess(ViewItemGroupObject);
        
        ViewItemGroupObject._vieweperm = MainObject.do_IsActionMenuPermission(ViewItemGroupObject.access, 'ITEM GROUP', 'view');
        ViewItemGroupObject._createperm = MainObject.do_IsActionMenuPermission(ViewItemGroupObject.access, 'ITEM GROUP', 'create');
        ViewItemGroupObject._editperm = MainObject.do_IsActionMenuPermission(ViewItemGroupObject.access, 'ITEM GROUP', 'edit');
        ViewItemGroupObject._deleteperm = MainObject.do_IsActionMenuPermission(ViewItemGroupObject.access, 'ITEM GROUP', 'delete');

        ViewItemGroupObject._accountmapvieweperm = MainObject.do_IsActionMenuPermission(ViewItemGroupObject.access, 'VIEW ITEMS UNDER GROUP', 'view');
        ViewItemGroupObject._accountmapmenuid = MainObject.do_IsActionMenuPermission(ViewItemGroupObject.access, 'VIEW ITEMS UNDER GROUP', 'menuid');
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                ViewItemGroupObject.rowid = response[0].RowId;
    ...
    do_loadwarehouselocation: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", LocationCd: '', CoCd: $("#ddlCompany").val(), LocationDesc: '', WhId: ViewItemGroupObject.rowid, AisleNo: -1, RackNo: -1, SelfNo: -1, BinNo: "-1", created_by: $("#txt").val(), RowId: -1 },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    ViewItemGroupObject.do_loadwarehouselocationData(obj);
                },
                error: function () {
...
function saveFinal(imgId) {
    ...
    if (parseInt(ViewItemGroupObject.rowid) > 0) {
        $.ajax({

            url: apiurl + 'api/InventoryItemGroupoperation',
            type: 'POST',
            data: { p_mode: "update", RowId: parseInt(ViewItemGroupObject.rowid), GroupCd: GroupCd, CoCd: CoCd, GroupDesc: GroupDesc, ItemGroupNo: ItemGroupNo, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
...
var otherWindow = function (countryCd, mode) {
    ...
    if (localStorage.budgetgrouprowid) {
        if (mode == "viewitem") {
            ...
            localStorage.menu_id_premission = ViewItemGroupObject._accountmapmenuid[1];
            localStorage.clickedmenu_id = ViewItemGroupObject._accountmapmenuid[1];
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == "viewlocation") {
        ViewItemGroupObject.rowid = rowId;
        showmodalview();

    }
    if (mode == 'add') {
        companylogo = "";
        ViewItemGroupObject.rowid = '-1';
        showmodal();
    ...
    if (mode == 'edit') {
        ...
        $('#txtCode').prop("disabled", true);
        ViewItemGroupObject.rowid = rowId;
        ViewItemGroupObject.do_loaddataedit(rowId);
...
function PopulateState() {
    ViewItemGroupObject.do_populateLocationDropdown('STATE', $('#ddPincodeCountry').val(), $('#ddPincodeCounty'));
}
function PopulateCity() {
    ViewItemGroupObject.do_populateLocationDropdown('CITY', $('#ddPincodeCounty').val(), $('#ddPincodeCity'));
}
