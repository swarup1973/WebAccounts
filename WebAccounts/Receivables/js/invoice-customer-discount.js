...
var InvoiceDiscountObject = {
    ...
    do_init: () => {
        ...
        if (queryString('menuid') != undefined || queryString("menuid") != null) {
            InvoiceDiscountObject.menuid = queryString("menuid");
    ...
    do_getUserPagepermission: () => {
        ...
        if (localStorage.menu_id_premission != undefined || localStorage.menu_id_premission != null) {
            InvoiceDiscountObject._createperm = MainObject.do_IsActionMenuPermission(InvoiceDiscountObject.access, 'SALES INVOICE DISCOUNT', 'create');
            InvoiceDiscountObject._editperm = MainObject.do_IsActionMenuPermission(InvoiceDiscountObject.access, 'SALES INVOICE DISCOUNT', 'edit');
            InvoiceDiscountObject._deleteperm = MainObject.do_IsActionMenuPermission(InvoiceDiscountObject.access, 'SALES INVOICE DISCOUNT', 'delete');
        }
    },
    ...
    do_loadinvoicediscount: () => {
        ...
        $.ajax({
            type: "POST",
            url: "Invoice-Customer-Discount.aspx/loadlist",
            data: JSON.stringify(_passdata),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
    ...
    do_populateVendorItemPrice: (obj) => {
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                if (!InvoiceDiscountObject._deleteperm[0]) {
                    $('#invcd_remove').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
                    $('#invcd_remove').prop("disabled", true);
                    $('#invcd_remove').attr('title', 'do not have delete permission!!!');
        ...
        if (!InvoiceDiscountObject._createperm[0]) {
            $('#invcd_add').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#invcd_add').prop("disabled", true);
            $('#invcd_add').attr('title', 'do not have permission to Add New Record!!!');
            table.button(0).action(function () {
                this.active(false);
            });
        }
        if (!InvoiceDiscountObject._editperm[0]) {
            $('#invcd_Edit').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#invcd_Edit').prop("disabled", true);
            $('#invcd_Edit').attr('title', 'do not have permission to Edit Record!!!');
        ...
        if (!InvoiceDiscountObject._deleteperm[0]) {
            $('#invcd_remove').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#invcd_remove').prop("disabled", true);
            $('#invcd_remove').attr('title', 'do not have permission to delete Record!!!');
            table.button(2).action(function () {
                this.active(false);
            });
        }
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            type: "POST",
            url: "Invoice-Customer-Discount.aspx/doedit",
            data: JSON.stringify(_passdata),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
                    ...
                    for (var key in objnew) {
                        ...
                        if (attrName.toLowerCase() == "table") {
                            if (objnew[key].length > 0) {
                                ...
                                $('#txt_minamt').val(objnew[key][0].MinAmt);
                                $('#txt_maxamt').val(objnew[key][0].MaxAmt);
...
var savedata = function () {
    ...
    min = $('#txt_minamt').val();
    if ($('#txt_maxamt').val() == '' || $('#txt_maxamt').val() == '0') max = $('#txt_minamt').val() + 1;
    else max = $('#txt_maxamt').val();
    ...
    if ($('#txt_invoicediscountcode').val() == '') {
    ...
    else if ($('#txt_minamt').val() == '') {
        validate = false;
        $.alertable.alert(`Minimum Amount required.`);
        $("#txt_minamt").focus();
        return false;
    }
    else if ($('#txt_minamt').val() == '0' || $('#txt_minamt').val() == '0.00') {
        validate = false;
        $.alertable.alert(`Minimum Amount should be greater then 0.`);
        $("#txt_minamt").focus();
        return false;
    }
        ...
        validate = false;
        $.alertable.alert(`Maximum Amount Cannot Be Less Then  Minimum Amount.`);
        $("#txt_maxamt").focus();
        return false;
    }
    ...
    /*else if (($('#txt_discountamount').val() != '' || $('#txt_discountamount').val() != '0' || $('#txt_discountamount').val() != '0.00') && ($('#txt_discount').val() != '') || $('#txt_discount').val() != '0' || $('#txt_discount').val() != '0.00') {
        ...
        $.ajax({
            type: "POST",
            url: "Invoice-Customer-Discount.aspx/docheckcode",
            data: _data,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
    ...
    if (validate == true) {
        ...
        _data["minamt"] = $('#txt_minamt').val();
        _data["maxamt"] = $('#txt_maxamt').val();
        ...
        var _url = "Invoice-Customer-Discount.aspx/doSave";
        $.ajax({
            type: "POST",
            url: _url,
            data: JSON.stringify(_passdata),
        ...
        $.alertable.custconfirm(`Are you want to delete the Invoice Discount ?`, ``, `Yes`, `No`)
            .then(
                function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "Invoice-Customer-Discount.aspx/dodelete",
                        data: _data,
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        async: false,
