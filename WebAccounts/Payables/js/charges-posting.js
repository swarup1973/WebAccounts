...
var ChargesPostingOverviewObject = {
    ...
    do_getUserPagepermission: () => {
        ...
        ChargesPostingOverviewObject._vieweperm = MainObject.do_IsActionMenuPermission(ChargesPostingOverviewObject.access, 'POSTING SETUP', 'view');
        ChargesPostingOverviewObject._createperm = MainObject.do_IsActionMenuPermission(ChargesPostingOverviewObject.access, 'POSTING SETUP', 'create');
        ChargesPostingOverviewObject._editperm = MainObject.do_IsActionMenuPermission(ChargesPostingOverviewObject.access, 'POSTING SETUP', 'edit');
        ChargesPostingOverviewObject._deleteperm = MainObject.do_IsActionMenuPermission(ChargesPostingOverviewObject.access, 'POSTING SETUP', 'delete');

        if (ChargesPostingOverviewObject._createperm[0] == true){
        ...
        success: function (response) {
            ...
            if (response[0].msg == "true") {
                ...
                $.alertable.alert(`Data updated successfully.`, ``, `Ok`, ``).then(function () {
                    localStorage.menu_id_premission=localStorage.chargesoverviewmenuid;
...
function cancelbtn(){
    localStorage.menu_id_premission=localStorage.chargesoverviewmenuid;
    location.href='charges-overview.aspx';
}