...
$(document).ready(function () {
    ...
    localStorage.chargesoverviewmenuid=localStorage.menu_id_premission;
    //ChargesOverviewObject.do_loaddata();

    
    ChargesOverviewObject.do_loaddata();
    ChargesOverviewObject.do_getUserPagepermission();
...
var ChargesOverviewObject = {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    ChargesOverviewObject.do_populateData(obj);
                },
                error: function (err) {
    ...
    do_populateData: (obj) => {
        ...
        table.on('select', function () {
            ...
            if (selectedRows == 1) {
                ...
                if (!ChargesOverviewObject._deleteperm[0]) {
        ...
        if (!ChargesOverviewObject._createperm[0]) {
        ...
        if (!ChargesOverviewObject._editperm[0]) {
        ...
        if (!ChargesOverviewObject._vieweperm[0]) {
        ...
        if (!ChargesOverviewObject._postingvieweperm[0]) {
            $('#country_overview_PostingSetup').addClass('btn btn-secondary buttons-selected buttons-remove disabled');
            $('#country_overview_PostingSetup').prop("disabled", true);
            $('#country_overview_PostingSetup').attr('title', 'do not have permission to view !!!');
    ...
    do_getUserPagepermission: () => {
        MainObject.do_getuserpageaccess(ChargesOverviewObject);
        
        ChargesOverviewObject._vieweperm = MainObject.do_IsActionMenuPermission(ChargesOverviewObject.access, 'CHARGES', 'view');
        ChargesOverviewObject._createperm = MainObject.do_IsActionMenuPermission(ChargesOverviewObject.access, 'CHARGES', 'create');
        ChargesOverviewObject._editperm = MainObject.do_IsActionMenuPermission(ChargesOverviewObject.access, 'CHARGES', 'edit');
        ChargesOverviewObject._deleteperm = MainObject.do_IsActionMenuPermission(ChargesOverviewObject.access, 'CHARGES', 'delete');


        ChargesOverviewObject._postingvieweperm = MainObject.do_IsActionMenuPermission(ChargesOverviewObject.access, 'POSTING SETUP', 'view');
        ChargesOverviewObject._postingmenuid = MainObject.do_IsActionMenuPermission(ChargesOverviewObject.access, 'POSTING SETUP', 'menuid');
    ...
    do_loaddataedit: (id) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                ChargesOverviewObject.rowid = response[0].RowId;
                $("#txtCode").val(response[0].ChargCd);
                $("#txtDescription").val(response[0].ChargesDesc);
    ...
    do_loadwarehouselocation: () => {
        ...
        _data = JSON.stringify({ cocd: $("#ddlCompany").val() }),
            $.ajax({
                ...
                type: 'POST',
                data: { p_mode: "getlist", LocationCd: '', CoCd: $("#ddlCompany").val(), LocationDesc: '', WhId: ChargesOverviewObject.rowid, AisleNo: -1, RackNo: -1, SelfNo: -1, BinNo: "-1", created_by: $("#txt").val(), RowId: -1 },
                contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (response) {
                    var obj = response;
                    ChargesOverviewObject.do_loadwarehouselocationData(obj);
                },
                error: function () {
...
var savedata = function () {
    ...
    else if ($('#txtDescription').val().length < 1) {
        validate = false;
        gotfocus($("#txtDescription"));
...
function saveFinal(imgId) {
    ...
    Purchase_CostPostType = 'C';
    Purchase_DebitPost = '-1';
    Sales_CreditPost = '-1';
    ChargCd = $("#txtCode").val();
    ChargesDesc = $("#txtDescription").val();
    
   
    
    if (parseInt(ChargesOverviewObject.rowid) > 0) {
        $.ajax({

            url: apiurl + 'api/PayablesChargesOperation',
            type: 'POST',
            data: { mode: "update", RowId: parseInt(ChargesOverviewObject.rowid), CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add, ChargCd: ChargCd, ChargesDesc: ChargesDesc, Purchase_CostPostType: Purchase_CostPostType, Purchase_DebitPost: Purchase_DebitPost, Sales_CreditPost: Sales_CreditPost },
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            success: function (response) {
        ...
        $.ajax({
            url: apiurl + 'api/PayablesChargesOperation',
            type: 'POST',
            data: { mode: "create", RowId: -1, CoCd: CoCd, IsBlock: IsBlock, created_by: created_by, creator_MAC_add: creator_MAC_add, ChargCd: ChargCd, ChargesDesc: ChargesDesc, Purchase_CostPostType: Purchase_CostPostType, Purchase_DebitPost: Purchase_DebitPost, Sales_CreditPost: Sales_CreditPost },
...
var otherWindow = function (countryCd, mode) {
    ...
    localStorage.chargesOverViewid = countryCd;
    //$('#addressbook').DataTable().rows('.selected').data()[0].AddressCode
    if (localStorage.chargesOverViewid) {
        if (mode == "POSTINGSETUP") {

            localStorage.menu_id_premission = ChargesOverviewObject._postingmenuid[1];
            localStorage.clickedmenu_id = ChargesOverviewObject._postingmenuid[1];
...
var roleaction = function (rowId, mode) {
    ...
    if (mode == "viewlocation") {
        ChargesOverviewObject.rowid = rowId;
        showmodalview();

    }
    if (mode == 'add') {
        companylogo = "";
        ChargesOverviewObject.rowid = '-1';
        showmodal();
    ...
    if (mode == 'edit') {
        ...
        $('#txtCode').prop("disabled", true);
        ChargesOverviewObject.rowid = rowId;
        ChargesOverviewObject.do_loaddataedit(rowId);
...
function PopulateState() {
    ChargesOverviewObject.do_populateLocationDropdown('STATE', $('#ddPincodeCountry').val(), $('#ddPincodeCounty'));
}
function PopulateCity() {
    ChargesOverviewObject.do_populateLocationDropdown('CITY', $('#ddPincodeCounty').val(), $('#ddPincodeCity'));
}
