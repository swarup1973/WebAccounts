...
namespace WebAccounts.Buisness
{
    public class customeritemprice
    {
        ...
        public string _mrp = string.Empty;
        ...
        public DataSet getcustomerdetails(string str_ConnString, ref string str_catchmessage)
        {
            DataSet ds = null;
            try
            {
                List<SqlParameter> SqlRecordParams = new List<SqlParameter>();
                SqlRecordParams.Add(new SqlParameter("@p_custcd", SqlDbType.VarChar) { Value = ((string.Empty == this._vendorcode) ? (object)DBNull.Value : this._vendorcode) });
                SqlRecordParams.Add(new SqlParameter("@CoCd", SqlDbType.VarChar) { Value = ((string.Empty == this._CoCd) ? (object)DBNull.Value : this._CoCd) });

                ds = DataHelper.ExecuteDataset(str_ConnString, "getCustomerdetails", SqlRecordParams.ToArray());
            }
        ...
        public DataSet Operation(string str_ConnString, ref string str_catchmessage)
        {
            ...
            try
            {
                ...
                SqlRecordParams.Add(new SqlParameter("@p_mrp", SqlDbType.VarChar) { Value = ((string.Empty == this._priceperunit) ? (object)DBNull.Value : Convert.ToDecimal(this._mrp)) });
                ...
                ds = DataHelper.ExecuteDataset(str_ConnString, "CustomerItemPrice_operation", SqlRecordParams.ToArray());
            }
            catch (Exception expErr)
            {
                ds = null;
